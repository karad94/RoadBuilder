...
def get_line_dict(start, direction, length, factor):
    ...
    dict['end'] = [get_int(dict['start'][0] + length * factor * math.cos(angle)),
                   get_int(dict['start'][1] - length * factor * math.sin(angle))]
    return dict
...
def get_right_curve_dict(start, direction, radius, arcLength, factor):
    ...
    dict['start'] = [get_int(start[0] - radius_pix + radius_pix * math.sin(direktion_angle)),
                     get_int(start[1] - radius_pix + radius_pix * math.cos(direktion_angle))]
    ...
    middle_point = [get_int(start[0] + radius_pix * math.sin(direktion_angle)),
                    get_int(start[1] + radius_pix * math.cos(direktion_angle))]
    dict['end'] = [get_int(middle_point[0] + radius_pix * math.cos(angle)),
                   get_int(middle_point[1] - radius_pix * math.sin(angle))]
...
def get_parking_area_dict(start, direction, length, right, left, factor):
    ...
    dict['end'] = [get_int(start[0] + length * factor * math.sin(angle)),
                   get_int(start[1] + length * factor * math.cos(angle))]
    return dict


def get_intersection_dict(start, direction, text, length, open_intersections, oneway, stopped_lanes, factor):
    """
    ...
    dict['length'] = length
    dict['oneway'] = oneway
    dict['stopped_lanes'] = stopped_lanes
    angle = math.radians(dict['direction'])
    if text in ('Rechtskurve', 'RIGHT'):
        dict['text'] = 'RIGHT'
        dict['endDirection'] = direction - 90
        middle_point = [get_int(start[0] + arm_length * math.cos(angle)),
                        get_int(start[1] - arm_length * math.sin(angle))]
        angle = math.radians(dict['endDirection'])
        dict['end'] = [get_int(middle_point[0] + arm_length * math.cos(angle)),
        ...
        open_end1 = [get_int(middle_point[0] + arm_length * math.cos(angle)),
                     get_int(middle_point[1] - arm_length * math.sin(angle))]
        open_direction_2 = open_direction_1 + 90
        angle = math.radians(open_direction_2)
        open_end_2 = [get_int(middle_point[0] + arm_length * math.cos(angle)),
                      get_int(middle_point[1] - arm_length * math.sin(angle))]
        open_intersections.append({'openEnd': open_end1, 'openDirection': open_direction_1 + 180, 'newEnd': open_end_2,
                                   'newDirection': open_direction_2, 'status': True, 'type': 'right',
                                   'radius': length / 2})
        open_intersections.append({'openEnd': open_end_2, 'openDirection': open_direction_2 + 180, 'newEnd': open_end1,
                                   'newDirection': open_direction_1, 'status': True, 'type': 'left',
                                   'radius': length / 2})
        ...
        open_end_2 = [get_int(middle_point[0] + arm_length * math.cos(angle)),
                      get_int(middle_point[1] - arm_length * math.sin(angle))]
        open_intersections.append({'openEnd': open_end1, 'openDirection': open_direction_1 + 180, 'newEnd': open_end_2,
                                   'newDirection': open_direction_2, 'status': True, 'type': 'left',
                                   'radius': length / 2})
        open_intersections.append({'openEnd': open_end_2, 'openDirection': open_direction_2 + 180, 'newEnd': open_end1,
                                   'newDirection': open_direction_1, 'status': True, 'type': 'right',
                                   'radius': length / 2})
        ...
        middle_point = [get_int(start[0] + arm_length * math.cos(angle)),
                        get_int(start[1] - arm_length * math.sin(angle))]
        dict['end'] = [get_int(start[0] + 2 * arm_length * math.cos(angle)),
                       get_int(start[1] - 2 * arm_length * math.sin(angle))]
        # calculate the open ends of the intersetion
        open_direction_1 = dict['endDirection'] - 90
        angle = math.radians(open_direction_1)
        open_end1 = [get_int(middle_point[0] + arm_length * math.cos(angle)),
        ...
        open_end_2 = [get_int(middle_point[0] + arm_length * math.cos(angle)),
                      get_int(middle_point[1] - arm_length * math.sin(angle))]
        open_intersections.append({'openEnd': open_end1, 'openDirection': open_direction_1 + 180, 'newEnd': open_end_2,
                                   'newDirection': open_direction_2, 'status': True, 'type': 'straight',
                                   'radius': length / 2})
        open_intersections.append({'openEnd': open_end_2, 'openDirection': open_direction_2 + 180, 'newEnd': open_end1,
                                   'newDirection': open_direction_1, 'status': True, 'type': 'straight',
                                   'radius': length / 2})
...
def get_traffic_island_dict(start, direction, zebra_length, island_width, curve_area_length, curvature, left_lanes,
                            factor):
    """
