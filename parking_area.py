...
class ParkingAreaWindow(QWidget):
    ...
    def __init__(self, parent_window):
        ...
        self.form_group_box.move(250, 0)
        ...
        self.add_parking_spot.resize(100, 150)
    ...
    def finish_button_clicked(self):
        ...
        if self.parking_dict['left'] or self.parking_dict['right']:
            self.parent_window.append_road_element(
                get_parking_area_dict(self.road[-1]['end'], self.road[-1]['endDirection'], self.parking_dict['length'],
                                      self.parking_dict['right'], self.parking_dict['left'], self.factor))
    ...
    def add_parking_spot_clicked(self):
        ...
        if self.park_start.text() and self.number_of_parking_spots.text():
            if float(self.park_start.text()) >= 0 and float(self.number_of_parking_spots.text()) > 0:
                ...
                new_end = dict['start'] + dict['number'] * self.parking_spot_size[dict['type']][0] + 2 * \
                          self.parking_spot_size[dict['type']][1]
                for spots in self.parking_dict[site]:
                    existing_end = spots['start'] + spots['number'] * self.parking_spot_size[spots['type']][0] + 2 * \
                                   self.parking_spot_size[spots['type']][1]
