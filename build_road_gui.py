...
from PyQt5.QtWidgets import QApplication, QMainWindow, QPushButton, QLabel, QLineEdit, QMessageBox, QFileDialog, \
    QFormLayout, QGroupBox, QShortcut, QWidget

import intersection
import parking_area
import select_line_style
import traffic_island
...
class Window(QMainWindow):
    ...
    def __init__(self):
        ...
        # road list
        self.road = []
        self.road.append({'name': 'firstElement', 'start': self.start, 'end': self.end, 'direction': self.direction,
                          'endDirection': self.direction})
        ...
        self.end_label = QLabel(
            f'x: {self.road[-1]["end"][0] / self.factor - 100}, y: {self.road[-1]["end"][1] / self.factor - 100}')
        self.direction_label = QLabel(f'{self.road[-1]["endDirection"] % 360}°')

        form = QFormLayout()
        form.setVerticalSpacing(0)
        form.addRow(QLabel('Start:'),
                    QLabel(f'x: {self.start[0] / self.factor - 100}, y: {self.start[1] / self.factor - 100}'))
        ...
        self.lane_amount = QLineEdit(self)
        self.lane_amount.move(screen_width - button_width - 20, 200)
        self.lane_amount.setToolTip('Anzahl an Fahrspuren')
        self.lane_amount.setPlaceholderText('Fahrspuren')
        self.lane_amount.setFixedWidth(button_width)
        self.lane_amount.setValidator(QtGui.QDoubleValidator())
        ...
        self.line_button.move(screen_width - button_width - 20, 200)
        ...
        self.blocked_area_button = QPushButton('Hindernis einfügen / Lücke', self)
        self.blocked_area_button.setToolTip(
            'Es wird eine Gerade mit Hindernis oder eine Lücke (Track_generator) eingefügt')
    ...
    def resizeEvent(self, event):
        """
        Is called when the main window gets resized.
        ...
        self.lane_amount.move(window_width - button_width - 20, 230)
        self.line_length.move(window_width - button_width - 20, 260)

        self.line_button.move(window_width - button_width - 20, 290)
        self.zebra_button.move(window_width - button_width - 20, 320)
        self.blocked_area_button.move(window_width - button_width - 20, 350)

        self.radius.move(window_width - button_width - 20, 410)
        self.arc_length.move(window_width - button_width - 20, 440)
        self.right_curve_button.move(window_width - button_width - 20, 470)
        self.left_curve_button.move(window_width - button_width - 20, 500)
    ...
    def save_python_button_clicked(self):
        ...
        if not len(self.road) > 1:
            ...
            return
        file, _ = QFileDialog.getSaveFileName(self, 'Geben Sie einen Speicherort an.',
                                              '/opt/.ros/kitcar-gazebo-simulation/simulation/models/env_db',
                                              'Python Files (*.py)')
        if file == '':
            return
    ...
    def load_python_button_clicked(self):
        ...
        """
        file, _ = QFileDialog.getOpenFileName(self, 'Wählen Sie die Datei aus.',
                                              '/opt/.ros/kitcar-gazebo-simulation/simulation/models/env_db',
                                              'Python Files (*.py)')
        if file == '':
            return
    ...
    def save_xml_button_clicked(self):
        ...
        xml_writer(self.road, file, self.factor, self.lane_amount.text())
        QMessageBox.about(self, 'Information', 'Die Strecke wurde gespeichert')
    ...
    def xml_preview_button_clicked(self):
        if not len(self.road) > 1:
            QMessageBox.about(self, 'Warning', 'Es ist keine Strecke vorhanden!')
            return
        xml_preview(self, self.lane_amount.text())

    def line_button_clicked(self):
        """
        Insert a straigtht element to the road.
        """
        if self.line_length.text():
            if float(self.line_length.text()) > 0:
                self.append_road_element(
                    get_line_dict(self.road[-1]['end'], self.road[-1]['endDirection'], float(self.line_length.text()),
                                  self.factor))
    ...
    def zebra_button_clicked(self):
        ...
        if self.line_length.text():
            if float(self.line_length.text()) > 0:
                self.append_road_element(
                    get_zebra_dict(self.road[-1]['end'], self.road[-1]['endDirection'], float(self.line_length.text()),
                                   self.factor))
    ...
    def blocked_area_button_clicked(self):
        ...
        if self.line_length.text():
            if float(self.line_length.text()) > 0:
                self.append_road_element(get_blocked_area_dict(self.road[-1]['end'], self.road[-1]['endDirection'],
                                                               float(self.line_length.text()), self.factor))
    ...
    def right_curve_button_clicked(self):
        ...
        if self.radius.text() and self.arc_length.text():
            if float(self.radius.text()) > 0 and float(self.arc_length.text()) > 0:
                self.append_road_element(
                    get_right_curve_dict(self.road[-1]['end'], self.road[-1]['endDirection'], float(self.radius.text()),
                                         float(self.arc_length.text()), self.factor))
    ...
    def left_curve_button_clicked(self):
        ...
        if self.radius.text() and self.arc_length.text():
            if float(self.radius.text()) > 0 and float(self.arc_length.text()) > 0:
                self.append_road_element(
                    get_left_curve_dict(self.road[-1]['end'], self.road[-1]['endDirection'], float(self.radius.text()),
                                        float(self.arc_length.text()), self.factor))
    ...
    def delete_list_element(self):
        ...
        if self.list_widget.selectedItems():
            ...
            QMessageBox.about(self, 'Warning', 'Es ist kein Streckenelement ausgewählt.')
        ...
        for idx, element in enumerate(self.road):
            ...
            if idx > 0:
                ...
                if element['name'] == 'line':
                    self.road[idx] = get_line_dict(prev_element['end'], prev_element['endDirection'], element['length'],
                                                   self.factor)
                elif element['name'] == 'zebra':
                    self.road[idx] = get_zebra_dict(prev_element['end'], prev_element['endDirection'],
                                                    element['length'], self.factor)
                elif element['name'] == 'blockedArea':
                    self.road[idx] = get_blocked_area_dict(prev_element['end'], prev_element['endDirection'],
                                                           element['length'], self.factor)
                elif element['name'] == 'circleRight':
                    self.road[idx] = get_right_curve_dict(prev_element['end'], prev_element['endDirection'],
                                                          element['radius'], element['arcLength'], self.factor)
                elif element['name'] == 'circleLeft':
                    self.road[idx] = get_left_curve_dict(prev_element['end'], prev_element['endDirection'],
                                                         element['radius'], element['arcLength'], self.factor)
                elif element['name'] == 'parkingArea':
                    self.road[idx] = get_parking_area_dict(prev_element['end'], prev_element['endDirection'],
                                                           element['length'], element['right'], element['left'],
                                                           self.factor)
                elif element['name'] == 'trafficIsland':
                    self.road[idx] = get_traffic_island_dict(prev_element['end'], prev_element['endDirection'],
                                                             element['zebraLength'], element['islandWidth'],
                                                             element['curveAreaLength'], element['curvature'],
                                                             element['left_lanes'], self.factor)
                elif element['name'] == 'intersection':
                    self.road[idx] = get_intersection_dict(prev_element['end'], prev_element['endDirection'],
                                                           element['text'], element['length'],
                                                           self.open_intersections, element['oneway'],
                                                           element['stopped_lanes'],
                                                           self.factor)
                self.road[idx]['end'], self.road[idx]['endDirection'], self.road[idx]['skip_intersection'], \
                self.road[idx]['intersection_radius'] = check_for_intersection_connection(self.road[idx]['end'],
                                                                                          self.road[idx][
                                                                                              'endDirection'],
                                                                                          self.open_intersections)
                self.road[idx].update({'rightLine': element['rightLine'], 'middleLine': element['middleLine'],
                                       'leftLine': element['leftLine']})
    ...
    def insert_list_name(self, name):
        ...
        if name == 'line':
            ...
            self.list_widget.addItem('Hindernis / Lücke')
    ...
    def append_road_element(self, road_element):
        ...
        road_element['end'], road_element['endDirection'], road_element['skip_intersection'], road_element[
            'intersection_radius'] = check_for_intersection_connection(road_element['end'],
                                                                       road_element['endDirection'],
                                                                       self.open_intersections)
    ...
    def paintEvent(self, event):
        ...
        for element in self.road:
            ...
            if element['name'] == 'zebra':
                ...
                for x in range(5):
                    painter.drawLine(
                        get_int(element['start'][0] + ((4 * road_width - 2 * road_width * x) / 10) * math.sin(angle)),
                        get_int(element['start'][1] + ((4 * road_width - 2 * road_width * x) / 10) * math.cos(angle)),
                        get_int(element['end'][0] + ((4 * road_width - 2 * road_width * x) / 10) * math.sin(angle)),
                        get_int(element['end'][1] + ((4 * road_width - 2 * road_width * x) / 10) * math.cos(angle)))
                painter.setPen(QPen(Qt.gray, road_width, cap=Qt.FlatCap))
            ...
            if element['name'] == 'blockedArea':
                ...
                painter.drawLine(get_int(element['start'][0] + road_width / 4 * math.sin(angle)),
                                 get_int(element['start'][1] + road_width / 4 * math.cos(angle)),
                                 get_int(element['end'][0] + road_width / 4 * math.sin(angle)),
                                 get_int(element['end'][1] + road_width / 4 * math.cos(angle)))
                painter.setPen(QPen(Qt.gray, road_width, cap=Qt.FlatCap))
            ...
            if element['name'] == 'trafficIsland':
                ...
                # Right
                points.append([get_int(element['start'][0] + road_width / 4 * math.sin(angle)),
                               get_int(element['start'][1] + road_width / 4 * math.cos(angle))])
                # -
                points.append([get_int(points[-1][0] + element['curveAreaLength'] * self.factor / 2 * math.cos(angle)),
                               get_int(points[-1][1] - element['curveAreaLength'] * self.factor / 2 * math.sin(angle))])
                start_of_island = [
                    get_int(points[-1][0] + element['curveAreaLength'] * self.factor / 2 * math.cos(angle)),
                    get_int(points[-1][1] - element['curveAreaLength'] * self.factor / 2 * math.sin(angle))]
                # \
                points.append([get_int(start_of_island[0] + element['islandWidth'] / 2 * self.factor * math.sin(angle)),
                               get_int(
                                   start_of_island[1] + element['islandWidth'] / 2 * self.factor * math.cos(angle))])
                # -
                points.append([get_int(points[-1][0] + element['zebraLength'] * self.factor * math.cos(angle)),
                               get_int(points[-1][1] - element['zebraLength'] * self.factor * math.sin(angle))])
                end_of_island = [get_int(points[-1][0] - element['islandWidth'] / 2 * self.factor * math.sin(angle)),
                                 get_int(points[-1][1] - element['islandWidth'] / 2 * self.factor * math.cos(angle))]
                # /
                points.append(
                    [get_int(end_of_island[0] + element['curveAreaLength'] / 2 * self.factor * math.cos(angle)),
                     get_int(end_of_island[1] - element['curveAreaLength'] / 2 * self.factor * math.sin(angle))])
                # -
                points.append([get_int(points[-1][0] + element['curveAreaLength'] * self.factor / 2 * math.cos(angle)),
                               get_int(points[-1][1] - element['curveAreaLength'] * self.factor / 2 * math.sin(angle))])
            ...
            if element['name'] == 'intersection':
                ...
                points.append([get_int(element['start'][0] + (arm_length * math.cos(angle))),
                               get_int(element['start'][1] - (arm_length * math.sin(angle)))])
                middle_point = points[-1]
                # Back
                points.append([get_int(middle_point[0] - (arm_length * math.cos(angle))),
                               get_int(middle_point[1] + (arm_length * math.sin(angle)))])
                points.append(middle_point)
                # Left              
                points.append([get_int(middle_point[0] - (arm_length * math.sin(angle))),
                               get_int(middle_point[1] - (arm_length * math.cos(angle)))])
                points.append(middle_point)
                # Top
                points.append([get_int(middle_point[0] + (arm_length * math.cos(angle))),
                               get_int(middle_point[1] - (arm_length * math.sin(angle)))])
            ...
            if element['name'] == 'parkingArea':
                ...
                for dict in element['right']:
                    ...
                    points = []
                    points.append([get_int(start[0] + ((dict['start'] * self.factor + 5) * math.cos(angle))),
                                   get_int(start[1] - ((dict['start'] * self.factor + 5) * math.sin(angle)))])
                    # |
                    points.append([get_int(points[-1][0] + (road_width / 2 * math.sin(angle))),
                                   get_int(points[-1][1] + (road_width / 2 * math.cos(angle)))])
                    point = [get_int(points[-1][0] + (spot_heigth * math.cos(angle))),
                             get_int(points[-1][1] - (spot_heigth * math.sin(angle)))]
                    # \
                    points.append([get_int(point[0] + (spot_heigth * math.sin(angle))),
                                   get_int(point[1] + (spot_heigth * math.cos(angle)))])
                    # -
                    points.append([get_int(points[-1][0] + (dict['number'] * spot_length * math.cos(angle))),
                                   get_int(points[-1][1] - (dict['number'] * spot_length * math.sin(angle)))])
                    point = [get_int(points[-1][0] - (spot_heigth * math.sin(angle))),
                             get_int(points[-1][1] - (spot_heigth * math.cos(angle)))]
                    # /
                    points.append([get_int(point[0] + (spot_heigth * math.cos(angle))),
                                   get_int(point[1] - (spot_heigth * math.sin(angle)))])
                    # |
                    points.append([get_int(points[-1][0] - (road_width / 2 * math.sin(angle))),
                                   get_int(points[-1][1] - (road_width / 2 * math.cos(angle)))])
                    p = [QPoint(i[0], i[1]) for i in points]
