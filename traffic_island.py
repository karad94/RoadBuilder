...
class TrafficIslandWindow(QWidget):
    def __init__(self, parent_window):
        ...
        self.left_lanes = QLineEdit()
        self.left_lanes.setMinimumWidth(100)
        self.left_lanes.setValidator(QtGui.QDoubleValidator())
        ...
        form.addRow(QLabel('Linke Spuren:'), self.left_lanes)
        ...
        self.form_group_box.setMinimumSize(300, 200)
        self.form_group_box.move(0, 0)
    ...
    def finish_button_clicked(self):
        ...
        if self.island_width.text() and self.zebra_length.text() and self.curve_area_length.text() and self.curvature.text() and self.left_lanes.text():
            ...
            left_lanes = float(self.left_lanes.text())

            if island_width > 0 and zebra_length > 0 and curve_area_length > 0 and curvature >= 0 and curvature <= 1 and left_lanes >= 0:
                self.hide()
                self.parent_window.append_road_element(
                    get_traffic_island_dict(self.road[-1]['end'], self.road[-1]['endDirection'], zebra_length,
                                            island_width, curve_area_length, curvature, left_lanes, self.factor))
